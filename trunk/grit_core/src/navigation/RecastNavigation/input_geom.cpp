...
bool InputGeom::loadGfxBody(rcContext* ctx, std::vector<GfxBodyPtr> bodies)
{
	if (m_mesh)
	{
		delete m_chunkyMesh;
	...
	if (!m_mesh->convertGfxBody(bodies))
	{
		//ctx->log(RC_LOG_ERROR, "buildTiledNavigation: Could not load '%s'", body);
		return false;
	}
...
bool InputGeom::loadRigidBody(rcContext* ctx, std::vector<RigidBodyPtr> bodies)
{
	...
	if (!m_mesh->convertRigidBody(bodies))
	{
		//ctx->log(RC_LOG_ERROR, "buildTiledNavigation: Could not load '%s'", body);
		return false;
	}
...
void InputGeom::drawOffMeshConnections(duDebugDraw* dd, bool hilight)
{
	...
	for (int i = 0; i < m_offMeshConCount; ++i)
	{
		...
		duAppendCircle(dd, v[0],v[1]+0.2f,v[2], m_offMeshConRads[i], baseColor);//++
		duAppendCircle(dd, v[3],v[4]+0.2f,v[5], m_offMeshConRads[i], baseColor);//++
